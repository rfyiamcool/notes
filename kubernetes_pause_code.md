...
到此完成了 net, pid, ipc 的命名空间共享. 其实现原理简单命令, 就是创建一个 pause 容器, 然后业务容器在启动时使用共享命名空间的参数来启动容器.
...
等待进程退出后回收进程的 `task_struct` 对象, 避免出现子进程已退出, 但通过 `ps aux` 出现僵尸状态. 没学过 apue 进程方面同学, 可以学下 孤儿进程和僵尸进程是怎么一回事.
